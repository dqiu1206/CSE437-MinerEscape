...
public class SpeedPowerUp : PickableItemScript {
	...
	public override void Update () {
		if (player != null) {//Powerup has been activated
			if (counter > 0) {
				...
				player.GetComponent<PlayerMoveScript>().changeSpeedMultiplier(1);
				//Destroy(gameObject);
			} else {
	...
	public override void Activate (GameObject p) {
		player = p;
		player.GetComponent<PlayerMoveScript>().changeSpeedMultiplier(speedMultiplier);
	}
}
